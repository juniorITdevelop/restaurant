    ...
    <div class="work-area">
      ...
      <div class="table-record" v-for="(purchase, index) in purchases" :key="index" @click="goToInfo(purchase._id)">
        ...
        <div class="seventh-colon">
          <div class="btn complete" @click.stop="completePurchase(purchase._id, index)"></div>
          <div class="btn delete" @click.stop="deletePurchase(purchase._id)"></div>
        </div>
      </div>
    </div>
    <div class="add-order" :class="{activePopup: formActive}">
      <form class="form-order">
        <div v-for="(item,index) in newPurchase.goods" :key="index" class="dish-record">
          <input type="text" placeholder="Запис списку" v-model="newPurchase.goods[index]" />
           <div class="btn delete" @click="deletePurchaseItem(index)" style="margin-left: 10px; padding: 0 20px;"></div>
        </div>
        <div class="btn add" @click="addPurchaseItem"></div>
...
export default {
  ...
  methods: {
    ...
    goToInfo(purchaseId){
      this.$router.push({path: '/purchaseInfo/'+ purchaseId});
    },
    showPopup() {
          ...
          this.$store.dispatch('loadCurrentUserData', user.userId)
           .then(res=>{
                  let obj = {
                        ...
                        action: 'Створено заявка на закпку: '
                 }
