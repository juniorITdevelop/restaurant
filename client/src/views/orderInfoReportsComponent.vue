<template>
  <div class="component-wrapper">
    <div class="header-panel">
      <h1 class="header-text">Інформація про виконане замовлення №: <span v-if="orderData[orderIndex]">{{orderData[orderIndex].orderNumber}}</span></h1>
      <div class="control-buttons">
      </div>
    </div>
    <div class="work-area">
      <div class="info-order" v-if="orderData[orderIndex]">
        <div class="item">Номер столика: <span>{{orderData[orderIndex].tableName}}</span></div>
        <div class="item">Кількість гостей: <span>{{orderData[orderIndex].guestsCount}}</span></div>
        <div class="item">Додаткова інформація: <span>{{orderData[orderIndex].additionalInfo}}</span></div>
        <div class="item">Час створення замовлення: <span>{{orderData[orderIndex].creationTime}}</span></div>
        <div class="item">Офіціант: <span>{{orderData[orderIndex].waiter}}</span></div>
        <div class="item">Номер замовлення: <span>{{orderData[orderIndex].orderNumber}}</span></div>
        <div class="item">Статус замовлення: <span>{{orderData[orderIndex].orderStatus}}</span></div>
        <div class="item">Тип подачі: <span>{{orderData[orderIndex].deliveryType}}</span></div>
        <div class="item">Ціна замовлення: <span>{{orderData[orderIndex].totalCost}}</span></div>
        ...
        <div class="header-dishes">
          ...
          <div>Знижка</div>
          <div>Час приготування</div>
          <div>Статус приготування</div>
          <div>Кількість</div>
          <div>Ціна</div>
        </div>

        <div class="dishes-table" v-for="(dish, ind) in orderData[orderIndex].dishes" :key="ind">
          <div>{{dish.categoryName}}</div>
          <div>{{dish.dishName}}</div>
          <div>{{dish.specialOffer}}</div>
          <div>{{dish.cookingTime}}</div>
          <div>{{dish.cookingStatus}}</div>
          <div>{{dish.count}}</div>
          <div>{{dish.cost}}</div>
...
export default {
  ...
  created: function(){
    this.$store.dispatch("loadStatistics").then(response => {
      this.orderData = this.$store.getters.getStatistics.orders;
