<template>
  <div class="component-wrapper">
    <div class="header-panel">
      <h1 class="header-text">Інформація про виконану заявку на закупку інгредієнтів №: <span v-if="purchasesData[purchaseIndex]">{{purchasesData[purchaseIndex].purchaseNumber}}</span></h1>
    ...
    <div class="work-area">
      <div class="info-order" v-if="purchasesData[purchaseIndex]">
        ...
        <div  class="text">Фінальна вартість: <span>{{purchasesData[purchaseIndex].totalCost}} грн.</span></div>
...
export default {
  ...
  created: function(){
    this.$store.dispatch("loadStatistics").then(response => {
      this.purchasesData = this.$store.getters.getStatistics.purchases;
